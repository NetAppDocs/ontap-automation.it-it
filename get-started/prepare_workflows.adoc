...
È possibile copiare l'esempio JSON in un file e salvarlo localmente. Il comando curl si riferisce al file utilizzando il `--data` parametro con il valore che indica il nome del file con un `@` prefisso.

.Incollare nel terminale dopo l'esempio di arricciatura
Per prima cosa è necessario copiare e incollare l'esempio di arricciatura nella shell del terminale. Quindi modificare l'esempio per rimuovere completamente `--data` parametro alla fine e sostituirlo con l'attrezzo `--data-raw` parametro. Infine, copiare e incollare nell'esempio JSON in modo che segua il comando curl e il parametro aggiornato.
