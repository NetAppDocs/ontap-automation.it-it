...
.Abilitazione anti-ransomware
È possibile controllare il funzionamento della funzionalità di abilitazione della Autonomous Ransomware Protection (ARP). Ciò include il recupero e la modifica delle impostazioni di configurazione. Questo tipo di risorsa è stato introdotto con ONTAP 9.18.1.
...
.Criteri IPsec
Puoi usare questo set di chiamate API per gestire le policy in vigore per una distribuzione IPSec. Questo tipo di risorsa è nuovo con ONTAP 9.8. È stato aggiornato con la versione 9.19 per recuperare le policy utilizzate per la rete del cluster.

.Associazioni di protezione IPsec
Puoi usare questo set di chiamate API per gestire le associazioni di sicurezza attive per una distribuzione IPSec. Questo tipo di risorsa è nuovo con ONTAP 9.8. Sono state aggiornate con la versione 9.19 per recuperare le associazioni di sicurezza utilizzate per la rete del cluster.
...
.Autenticazione Web
L'autenticazione Web (WebAuthn) è uno standard web per l'autenticazione sicura degli utenti basata sulla crittografia a chiave pubblica. Con ONTAP, puoi gestire autenticazioni MFA resistenti al phishing tramite System Manager e l'ONTAP REST API. Questa funzionalità è stata aggiunta con ONTAP 9.16 e aggiornata con ONTAP 9.19.

.Whoami
È disponibile un endpoint per visualizzare i dettagli dell'utente autenticato, inclusi nome utente, ruolo e autorizzazioni. Questo tipo di risorsa è stato introdotto con ONTAP 9.19.
