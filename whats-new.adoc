...
La release ONTAP 9.19.1 include diversi aggiornamenti che continuano a migliorare e perfezionare la ONTAP REST API.

Sicurezza:: È stata aggiunta una nuova `whoami` funzionalità per recuperare le informazioni relative a nome utente, ruolo e autorizzazioni dell'utente connesso. Anche la funzionalità di autenticazione web (WebAuthn) è stata migliorata. IPsec è stato aggiornato per recuperare le policy e le associazioni di sicurezza utilizzate con la rete del cluster.
SAN:: La gestione delle mappe LUN è stata aggiornata con una funzionalità di aggiornamento migliorata.
...
Sicurezza:: Sono stati introdotti diversi miglioramenti alla sicurezza con l'API REST di ONTAP 9.18.1. È possibile utilizzare i nuovi endpoint di rete del cluster per visualizzare e aggiornare la configurazione di sicurezza della rete del cluster, inclusa la gestione dei certificati. Inoltre, è possibile amministrare la configurazione di sicurezza della rete HA per il traffico NVLog. Autonomous Ransomware Protection (ARP) è stata migliorata per supportare la visualizzazione e l'aggiornamento della configurazione di abilitazione automatica.
