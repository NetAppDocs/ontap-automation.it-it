...
La versione ONTAP 9.18.1 continua ad ampliare le capacità dell'API REST ONTAP con undici nuove chiamate API. I miglioramenti si concentrano principalmente sulla sicurezza, ma includono anche un miglioramento relativo all'amministrazione dello storage.

Sicurezza:: Con l'API REST ONTAP 9.18.1 sono stati introdotti due importanti miglioramenti alla sicurezza. È possibile utilizzare i nuovi endpoint API della rete del cluster per visualizzare e aggiornare la configurazione di sicurezza della rete del cluster. È incluso un solido supporto per la gestione dei certificati con la rete del cluster. Inoltre, è possibile visualizzare e aggiornare la configurazione di sicurezza della rete HA per il traffico NVLog.
